...
3. 本文、装飾、背景は data\sticky-note.json、位置とサイズは data\window-state.json へ自動保存されます。
...
Google Drive上では、Koyomadoと同様にローカルの待機プログラムを先に起動します。すでに罫彩が起動していれば二重起動せず、アプリ一式と本文・位置の両方の保存データを読み書きできる状態が安定するまで最大10分待ってから起動します。
配置場所を移動した場合は、移動後のアプリで「自動起動」を一度オンにし直してください。
1.5.1以前から自動起動を使っていた場合も、1.5.2への更新後に一度オンにし直してください。
