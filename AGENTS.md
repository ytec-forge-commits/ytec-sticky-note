...
- 本文・装飾・背景は実行ファイルと同じ場所の `data/sticky-note.json` が正本。隣の `.bak` は直前保存のバックアップ。
...
- 自動起動はKoyomadoと同様に `%LOCALAPPDATA%\\Y-TEC\\StickyNote` のローカル待機プログラムを先に起動する。二重起動を避け、Google Drive上のアプリ一式と既存データが読み書き可能な状態で安定するまで最大10分待つ。
- Rust製の自動起動待機プログラムはx64 MSVC CRTを静的リンクし、配布前に `scripts/check-startup-dependencies.ps1` で `VCRUNTIME`、`MSVCP`、Universal CRTの再配布DLLへ動的依存していないことを確認する。補助DLLの同時コピーを前提にしない。
...
- SignPath Foundationの採択前または署名処理を利用できない配布物は「未署名」と明記する。署名済みと未署名の成果物を同じ表現で公開しない。
