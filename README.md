...
本文・装飾・背景は、実行ファイルと同じ場所の `data/sticky-note.json` へ保存します。JSONは暗号化せず、装飾付き本文は箇条書き構造を保持するXAMLパッケージと互換用RTFをBase64表現で、検索しやすいプレーンテキストも同時に保存します。旧版のRTFだけのデータも読み込め、新形式を読めない場合はRTFへフォールバックします。ファイル更新は一時ファイル経由で行い、既存データを `.bak` へ退避してから置き換えます。v1から初めて移行する時は、上書きされない `sticky-note.json.v1.bak` も作成します。
