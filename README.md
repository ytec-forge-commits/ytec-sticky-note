...
USBメモリやGoogle Driveで持ち運ぶ場合は、EXEだけでなくフォルダー全体を移動してください。移動後の初回起動でWindows自動起動のパスも更新されます。リムーバブルドライブが未接続、またはクラウドドライブが未マウントのWindows起動時にはアプリを開始できません。
...
`artifacts/YTEC-Sticky-Note-win-x64/` と `artifacts/YTEC-Sticky-Note-1.0.0-win-x64.zip` を生成します。自己完結型のポータブルフォルダーなので、利用PCへの.NETランタイム導入は不要です。EXEだけを取り出さず、フォルダー全体を一緒に移動してください。
